...
When sending vote, for each of the next CONNECT_AHEAD slot leaders 'i':
  ...
  - CONNECTED: call fd_quic_conn_let_die(i*1000) so it doesn't die before
    we need it
...
during_housekeeping( fd_send_tile_ctx_t * ctx ) {
  ...
  if( ctx->housekeeping_ctr++ % MAP_STATS_PERIOD == 0UL ) {
    ...
    FD_LOG_INFO(("send_tile map check: %lu/%lu staked, %lu stale", staked_no_ci, map_real_cnt, stale_ci ));
  }
...
get_quic_conn( fd_send_tile_ctx_t * ctx,
  ...
  if( FD_LIKELY( entry ) ) {
    if( !entry->got_ci_msg ) ctx->metrics.get_conn_no_ci_msg++;
    else if( !entry->conn ) ctx->metrics.get_conn_entry_null_conn++;
    return entry->conn;
  }
...
quic_send( fd_send_tile_ctx_t  *  ctx,
  ...
  fd_quic_conn_let_die( conn, 10*1000UL );
}
...
after_frag( fd_send_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( kind==IN_KIND_TOWER ) ) {
    ...
    for( ulong i=SEND_TO_LEADER_CNT; i<CONNECT_AHEAD_CNT; i++ ) {
      ensure_conn_for_slot( ctx, poh_slot +  i, i*1000UL );
    }
...
metrics_write( fd_send_tile_ctx_t * ctx ) {
  ...
  FD_MCNT_SET( SEND, GET_CONN_NO_CI_MSG, ctx->metrics.get_conn_no_ci_msg );
  FD_MCNT_SET( SEND, GET_CONN_ENTRY_NULL_CONN, ctx->metrics.get_conn_entry_null_conn );
