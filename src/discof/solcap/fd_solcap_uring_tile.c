...
uring_sq_flush( fd_solcap_tile_ctx_t * ctx ) {
  ...
  FD_TEST( res==(int)ctx->sqe_pending );
  ctx->sqe_pending = 0UL;
}
...
frag_append( fd_solcap_tile_ctx_t * ctx,
  ...
  long const cur_free   = buf_max - (long)buf->off;
  ...
  long const tail_avail = (long)ctx->buf_free_cnt * buf_max;
...
returnable_frag( fd_solcap_tile_ctx_t * ctx,
  ...
  ctx->idle_cnt = -1;
  uchar const * data = fd_chunk_to_laddr_const( NULL, chunk );
...
during_housekeeping( fd_solcap_tile_ctx_t * ctx ) {
  ...
  if( ctx->sqe_pending ) uring_sq_flush( ctx );
  long t = fd_log_wallclock();
  static long prev_t = 0L;
  static ulong prev_off = 0UL;
  if( !prev_t ) prev_t = t;
  if( t-prev_t>(long)1e9 ) {
    FD_LOG_NOTICE(( "rate %g", (double)( ctx->file_off-prev_off ) ));
    prev_t   = t;
    prev_off = ctx->file_off;
  }
}
