...
after_credit( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !ctx->snapshot_init_done && strlen( ctx->genesis )>0 ) ) {
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( FD_UNLIKELY( !ctx->turbine_slot0 ) ) FD_LOG_ERR(( "replay tile has no turb_slot0 fseq" ));
...
metrics_write( fd_replay_tile_ctx_t * ctx ) {
  FD_MGAUGE_SET( REPLAY, TURBINE_SLOT, fd_fseq_query( ctx->turbine_slot ) );
  FD_MGAUGE_SET( REPLAY, TURBINE_SLOT0, fd_fseq_query( ctx->turbine_slot0 ) );
