...
fd_gui_run_boot_progress( fd_gui_t * gui, long now_nanos ) {
  ...
  fd_topo_tile_t const * replay = &gui->topo->tiles[ fd_topo_find_tile( gui->topo, "replay", 0UL ) ];
  volatile ulong * replay_metrics = fd_metrics_tile( replay->metrics );

  fd_topo_tile_t const * repair = &gui->topo->tiles[ fd_topo_find_tile( gui->topo, "repair", 0UL ) ];
  volatile ulong * repair_metrics = fd_metrics_tile( repair->metrics );

  ulong phase = snaprd_metrics[ MIDX( GAUGE, SNAPRD, STATE ) ];
  ulong snapshot_idx = ULONG_MAX;
  ...
  if( FD_UNLIKELY( phase >= 2UL /* FD_SNAPRD_STATE_READING_FULL_FILE */ ) ) {
    ...
    snapshot_idx = FD_GUI_BOOT_PROGRESS_FULL_SNAPSHOT_IDX;
  }
  ...
  if( FD_UNLIKELY( phase >= 8UL /* FD_SNAPRD_STATE_READING_INCREMENTAL_FILE */ ) ) {
    ...
    snapshot_idx = FD_GUI_BOOT_PROGRESS_INCREMENTAL_SNAPSHOT_IDX;
  }

  ulong _latest_turbine_slot = repair_metrics[ MIDX( GAUGE, REPAIR, LATEST_TURBINE_SLOT ) ];
  ulong _first_turbine_slot = repair_metrics[ MIDX( GAUGE, REPAIR, FIRST_TURBINE_SLOT ) ];
  ulong _latest_replay_slot = replay_metrics[ MIDX( GAUGE, REPLAY, SLOT ) ];
  ulong _latest_repair_slot = repair_metrics[ MIDX( GAUGE, REPAIR, LATEST_REPAIR_SLOT ) ];

  if( FD_UNLIKELY( _latest_replay_slot > 0UL ) ) {
    gui->summary.boot_progress.phase = FD_GUI_BOOT_PROGRESS_TYPE_CATCHING_UP;
  }

  if( FD_UNLIKELY( _latest_turbine_slot && _latest_replay_slot && ((long)_latest_turbine_slot - (long)_latest_replay_slot < 5L) ) ) {
...
#undef EMA_FILTER
    ...
    case FD_GUI_BOOT_PROGRESS_TYPE_CATCHING_UP: {
      ...
      gui->summary.boot_progress.catching_up_first_turbine_slot  = _first_turbine_slot;
      gui->summary.boot_progress.catching_up_latest_turbine_slot = _latest_turbine_slot;
      gui->summary.boot_progress.catching_up_latest_repair_slot  = _latest_repair_slot;
      gui->summary.boot_progress.catching_up_latest_replay_slot  = _latest_replay_slot;
      break;
    }
...
fd_gui_handle_snapshot_update( fd_gui_t *    gui,
  ...
  gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].slot = msg->slot;
  gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].peer_addr = msg->peer_addr;
  gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].peer_port = msg->peer_port;
