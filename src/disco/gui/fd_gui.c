...
fd_gui_new( void *             shmem,
  ...
  if( FD_UNLIKELY( gui->summary.is_full_client ) ) {
    ...
    gui->summary.boot_progress.loading_incremental_snapshot.reset_cnt = ULONG_MAX; /* ensures other fields are reset initially */
    gui->summary.boot_progress.loading_incremental_snapshot.read_path[ 0 ] = '\0';
    gui->summary.boot_progress.loading_incremental_snapshot.insert_path[ 0 ] = '\0';
  } else {
  ...
  memset( &gui->summary.gossip_network_stats, 0, sizeof(gui->summary.gossip_network_stats) );
...
fd_gui_run_boot_progress( fd_gui_t * gui, long now_nanos ) {
  ...
  ulong phase = snaprd_metrics[ MIDX( GAUGE, SNAPRD, STATE ) ];

  if( FD_UNLIKELY( phase >= 2UL /* FD_SNAPRD_STATE_READING_FULL_FILE */ ) ) {
    gui->summary.boot_progress.phase = FD_GUI_BOOT_PROGRESS_TYPE_LOADING_FULL_SNAPSHOT;
  }

  if( FD_UNLIKELY( phase >= 8UL /* FD_SNAPRD_STATE_READING_INCREMENTAL_FILE */ ) ) {
    gui->summary.boot_progress.phase = FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCREMENTAL_SNAPSHOT;
  }

  if( FD_UNLIKELY( phase >= 12UL /* FD_SNAPRD_STATE_SHUTDOWN */ ) ) {
...
#define HANDLE_SNAPSHOT_STATE(snapshot_type, snapshot_type_upper) \
      ulong _retry_cnt = snaprd_metrics[ MIDX( GAUGE, SNAPRD, snapshot_type_upper##_DOWNLOAD_RETRIES ) ]; \
      if( FD_UNLIKELY( gui->summary.boot_progress.loading_##snapshot_type##_snapshot.reset_cnt!=_retry_cnt ) ) { \
        gui->summary.boot_progress.loading_##snapshot_type##_snapshot.reset_time_nanos = now_nanos; \
        /* todo: init to ULONG_MAX? */ gui->summary.boot_progress.loading_##snapshot_type##_snapshot.reset_cnt = _retry_cnt; \
        /* reset ema filters */ \
        gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_throughput_ema = 0.; \
        gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_throughput_ema = 0.; \
        gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_throughput_ema = 0.; \
        gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_accounts_throughput_ema = 0.; \
      } \
      /* slot: todo ... send from snaprd */ \
      /* peer_{ip_addr|port}: todo ... send from snaprd */ \
      /* read_path: todo ... send from snaprd */ \
      /* insert_path: todo ... send from snapin */ \
      /**/ \
      /* read stage */ \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.total_bytes = snaprd_metrics[ MIDX( GAUGE, SNAPRD, snapshot_type_upper##_BYTES_TOTAL ) ]; \
      ulong _nanos_elapsed_since_last_sample = (ulong)fd_long_max(now_nanos - gui->summary.boot_progress.loading_##snapshot_type##_snapshot.sample_time_nanos, 1L); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.sample_time_nanos = now_nanos; \
      ulong _read_bytes = snaprd_metrics[ MIDX( GAUGE, SNAPRD, snapshot_type_upper##_BYTES_READ ) ]; \
      ulong _read_bytes_since_last_sample = _read_bytes - gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_bytes; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_throughput_ema = \
          (      FD_GUI_EMA_FILTER_ALPHA) * ((double)_read_bytes_since_last_sample / (double)_nanos_elapsed_since_last_sample) \
        + ( 1. - FD_GUI_EMA_FILTER_ALPHA) * (gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_throughput_ema); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_bytes = _read_bytes; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_remaining_nanos = (long)((double)(gui->summary.boot_progress.loading_##snapshot_type##_snapshot.total_bytes - _read_bytes) / gui->summary.boot_progress.loading_##snapshot_type##_snapshot.read_throughput_ema); \
      /* decompress stage */ \
      ulong _decompress_bytes = snapdc_metrics[ MIDX( GAUGE, SNAPDC, snapshot_type_upper##_COMPRESSED_BYTES_READ ) ]; \
      ulong _decompress_bytes_since_last_sample = _decompress_bytes - gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_decompressed_bytes; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_throughput_ema = \
          (      FD_GUI_EMA_FILTER_ALPHA) * ((double)_decompress_bytes_since_last_sample / (double)_nanos_elapsed_since_last_sample) \
        + ( 1. - FD_GUI_EMA_FILTER_ALPHA) * (gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_throughput_ema); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_compressed_bytes = _decompress_bytes; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_decompressed_bytes = snapdc_metrics[ MIDX( GAUGE, SNAPDC, snapshot_type_upper##_DECOMPRESSED_BYTES_READ ) ]; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_remaining_nanos = (long)((double)(gui->summary.boot_progress.loading_##snapshot_type##_snapshot.total_bytes - _decompress_bytes) / gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_throughput_ema); \
      /* insert stage */ \
      ulong _insert_bytes = snapin_metrics[ MIDX( GAUGE, SNAPIN, snapshot_type_upper##_BYTES_READ ) ]; \
      ulong _insert_bytes_since_last_sample = _insert_bytes - gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_bytes; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_throughput_ema = \
          (      FD_GUI_EMA_FILTER_ALPHA) * ((double)_insert_bytes_since_last_sample / (double)_nanos_elapsed_since_last_sample) \
        + ( 1. - FD_GUI_EMA_FILTER_ALPHA) * (gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_throughput_ema); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_bytes = _insert_bytes; \
      /* Use the latest compression ratio to estimate decompressed size */ \
      double _compression_ratio_estimate = (double)gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_decompressed_bytes/(double)gui->summary.boot_progress.loading_##snapshot_type##_snapshot.decompress_compressed_bytes; \
      ulong _total_size_estimate = (ulong)((double)gui->summary.boot_progress.loading_##snapshot_type##_snapshot.total_bytes * _compression_ratio_estimate); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_remaining_nanos = (long)((double)(_total_size_estimate - _insert_bytes) / gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_throughput_ema); \
      ulong _insert_accounts = snapin_metrics[ MIDX( GAUGE, SNAPIN, ACCOUNTS_INSERTED ) ]; \
      ulong _insert_accounts_since_last_sample = _insert_accounts - gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_accounts_current; \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_accounts_throughput_ema = \
          (      FD_GUI_EMA_FILTER_ALPHA) * ((double)_insert_accounts_since_last_sample / (double)_nanos_elapsed_since_last_sample) \
        + ( 1. - FD_GUI_EMA_FILTER_ALPHA) * (gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_accounts_throughput_ema); \
      gui->summary.boot_progress.loading_##snapshot_type##_snapshot.insert_accounts_current = _insert_accounts;
  ...
  if( FD_LIKELY( now>gui->next_sample_100millis ) ) {
    ...
    *gui->summary.tile_stats_reference = *gui->summary.tile_stats_current;
    ...
    if( FD_UNLIKELY( gui->summary.is_full_client && gui->summary.boot_progress.phase!=FD_GUI_BOOT_PROGRESS_TYPE_RUNNING ) ) {
      fd_gui_printf_gossip_network_stats( gui );
      fd_http_server_ws_broadcast( gui->http );
