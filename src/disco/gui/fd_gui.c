...
static void
fd_gui_run_boot_progress( fd_gui_t * gui, long now_ns ) {
  ...
  if( FD_UNLIKELY( phase >= 5UL /* FD_SNAPRD_STATE_READING_INCREMENTAL_FILE */ ) ) {
    gui->summary.boot_progress.phase = FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCR_SNAPSHOT;
  }

  switch ( gui->summary.boot_progress.phase ) {
    case FD_GUI_BOOT_PROGRESS_TYPE_JOINING_GOSSIP: {
      gui->summary.boot_progress.joining_gossip_elapsed_ns = now_ns - gui->summary.startup_time_nanos;
      break;
    }
    case FD_GUI_BOOT_PROGRESS_TYPE_LOADING_FULL_SNAPSHOT: {
      break;
    }
    case FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCR_SNAPSHOT: {
      break;
    }
...
fd_gui_handle_snapshot_update( fd_gui_t *    gui,
                               fd_restore_snapshot_update_t const * msg ) {
  (void)gui;
  FD_LOG_WARNING(("IN GUI RECVD SNAPSHOT: %lu", msg->slot));
  return;
}
