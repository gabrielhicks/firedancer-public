...
/* Adapted from "../../firedancer/topology.c" */
static void
repair_topo( config_t * config ) {
  resolve_gossip_entrypoints( config );

  ulong net_tile_cnt    = config->layout.net_tile_count;
  // ulong net_tile_cnt    = 1UL; /* TODO fixed here? */
  ulong shred_tile_cnt  = config->layout.shred_tile_count;
  // ulong shred_tile_cnt  = 1UL; /* TODO fixed here? */
  ulong quic_tile_cnt   = config->layout.quic_tile_count;
  // ulong quic_tile_cnt   = 1UL; /* TODO fixed here? */
  // ulong verify_tile_cnt = config->layout.verify_tile_count;
  // ulong verify_tile_cnt = 1UL;
  // ulong bank_tile_cnt   = config->layout.bank_tile_count;
  // ulong bank_tile_cnt   = 1UL; /* TODO fixed here? */
  // ulong exec_tile_cnt   = config->firedancer.layout.exec_tile_count;
  // ulong exec_tile_cnt   = 0UL; /* TODO fixed here? */
  // ulong writer_tile_cnt = config->firedancer.layout.writer_tile_count;
  // ulong resolv_tile_cnt = config->layout.resolv_tile_count;
  // (void)writer_tile_cnt;
  // (void)resolv_tile_cnt;

  // ulong empty_tile_cnt = fd_ulong_max( 0UL, fd_ulong_max( net_tile_cnt, fd_ulong_max( shred_tile_cnt, fd_ulong_max( quic_tile_cnt, fd_ulong_max( verify_tile_cnt, fd_ulong_max( bank_tile_cnt, exec_tile_cnt ) ) ) ) ) );
  // FD_LOG_NOTICE(( "net_tile_cnt %lu", net_tile_cnt ));

  // int enable_rpc = ( config->rpc.port != 0 );
  // int enable_rpc = 0; /* TODO new */
...
//  fd_topob_wksp( topo, "dedup_resolv" );
  ...
  // fd_topob_wksp( topo, "runtime_pub" ); /* TODO needed! */
  // fd_topob_wksp( topo, "banks"       ); /* TODO needed! */
  // fd_topob_wksp( topo, "bh_cmp" ); /* TODO needed! */
  // fd_topob_wksp( topo, "exec"        );
  // fd_topob_wksp( topo, "writer"      );
  // fd_topob_wksp( topo, "blockstore"  );
  fd_topob_wksp( topo, "fec_sets"    );
  // fd_topob_wksp( topo, "tcache"      ); /* TODO needed! */
  ...
  // fd_topob_wksp( topo, "funk" ); /* TODO needed! */
  fd_topob_wksp( topo, "slot_fseqs"  ); /* fseqs for marked slots eg. turbine slot */ /* TODO needed! */
...
//  /**/                 fd_topob_link( topo, "dedup_resolv", "dedup_resolv", 65536UL,                                  FD_TPU_PARSED_MTU,             1UL );
  ...
  // FOR(bank_tile_cnt)   fd_topob_link( topo, "replay_poh",   "replay_poh",   128UL,                                    (4096UL*sizeof(fd_txn_p_t))+sizeof(fd_microblock_trailer_t), 1UL  ); /* TODO why FOR(bank_tile_cnt) ? */
  ...
  // FOR(empty_tile_cnt)              fd_topob_tile( topo, "empty",   "empty",   "metric_in",  tile_to_cpu[ topo->tile_cnt ], 0,        0 );
  ...
  // FOR(quic_tile_cnt) for( ulong j=0UL; j<net_tile_cnt; j++ )
  //                     fd_topob_tile_in(  topo, "empty",    i,            "metric_in", "net_quic",     j,            FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED ); /* No reliable consumers of networking fragments, may be dropped or overrun */
  // for( ulong link_i = 0UL; link_i < topo->link_cnt; link_i++ ) { if( !strcmp( topo->links[ link_i ].name, "net_quic" ) ) topo->links[ link_i ].permit_no_consumers = 1; }

  // FOR(quic_tile_cnt)   fd_topob_tile_out( topo, "quic",    i,                         "quic_verify",  i                                                  );

  // FOR(quic_tile_cnt)   fd_topob_tile_out( topo, "quic",    i,                         "quic_net",     i                                                  );
  // FOR(quic_tile_cnt)   fd_topob_tile_out( topo, "empty",    i,                         "quic_net",     i                                                  );
  ...
  // FOR(verify_tile_cnt) fd_topob_tile_in(  topo, "empty",  i,            "metric_in", "gossip_verif", 0UL,          FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED ); /* No reliable consumers of networking fragments, may be dropped or overrun */
...
//  FOR(resolv_tile_cnt) fd_topob_tile_in(  topo, "resolv",  i,            "metric_in", "replay_resol", 0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  // /**/                 fd_topob_tile_in(  topo, "empty",   0UL,          "metric_in", "gossip_tower", 0UL,           FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED ); /* No reliable consumers of networking fragments, may be dropped or overrun */
  // /**/                 fd_topob_tile_in(  topo, "tower",   0UL,          "metric_in", "replay_tower", 0UL,           FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  // /**/                 fd_topob_tile_in(  topo, "empty",   0UL,          "metric_in", "replay_tower", 0UL,           FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );

  // /**/                 fd_topob_tile_out( topo, "tower",  0UL,                        "tower_replay", 0UL                                                   );
  // /**/                 fd_topob_tile_out( topo, "empty",  0UL,                        "tower_replay", 0UL                                                   );
  ...
  // FOR(exec_tile_cnt)   fd_topob_tile_in(  topo, "empty",    i,            "metric_in", "replay_exec",  i,            FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED    );
  ...
  // /**/                 fd_topob_tile_in ( topo, "empty",   0UL,         "metric_in", "gossip_send",   0UL,    FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED   );
  ...
  // /**/                 fd_topob_tile_out( topo, "empty",   0UL,                      "send_txns",     0UL                                            );
  ...
  // /**/                 fd_topob_tile_out( topo, "empty",   0UL,                       "pack_replay",  0UL                                            );
  ...
  // FOR(bank_tile_cnt)   fd_topob_tile_in ( topo, "empty",    0UL,         "metric_in",  "replay_poh",   i,      FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED   ); /* No reliable consumers of networking fragments, may be dropped or overrun */
  ...
  // /**/                 fd_topob_tile_out( topo, "empty",    0UL,                       "poh_shred",    0UL                                            );
  ...
  if( 1 ) {
    ...
    fd_topo_tile_t * shrdcp_tile = fd_topob_tile( topo, "shrdcp", "shredcap", "metric_in", tile_to_cpu[ topo->tile_cnt ], 0, 0 );
    ...
    fd_topob_tile_out( topo, "shrdcp",  0UL, "stake_out", 0UL );

    fd_topo_obj_t * root_slot_obj = fd_topob_obj( topo, "fseq", "slot_fseqs" );
    fd_topob_tile_uses( topo, shrdcp_tile, root_slot_obj, FD_SHMEM_JOIN_MODE_READ_WRITE );
    FD_TEST( fd_pod_insertf_ulong( topo->props, root_slot_obj->id, "root_slot" ) );
  }
  ...
  // FD_TEST( link_permit_no_producers( topo, "tower_replay" ) == 1UL           );
  FD_TEST( link_permit_no_producers( topo, "poh_shred"    ) == 1UL           );
  // FD_TEST( link_permit_no_producers( topo, "pack_replay"  ) == 1UL           );
  FD_TEST( link_permit_no_producers( topo, "send_txns"    ) == 1UL           );

  FD_TEST( link_permit_no_consumers( topo, "net_quic"     ) == quic_tile_cnt );
  // FD_TEST( link_permit_no_consumers( topo, "replay_exec"  ) == exec_tile_cnt );
  FD_TEST( link_permit_no_consumers( topo, "gossip_verif" ) == 1UL           );
  FD_TEST( link_permit_no_consumers( topo, "gossip_tower" ) == 1UL           );
  // FD_TEST( link_permit_no_consumers( topo, "replay_tower" ) == 1UL           );
  FD_TEST( link_permit_no_consumers( topo, "gossip_send"  ) == 1UL           );
  // FD_TEST( link_permit_no_consumers( topo, "replay_poh"   ) == bank_tile_cnt );
  // FD_TEST( link_permit_no_consumers( topo, "replay_notif" ) == 1UL           );
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    // if( ( !fd_topo_configure_tile( tile, config ) ) && ( !!strcmp( tile->name, "empty" ) ) ) {
...
repair_cmd_fn( args_t *   args FD_PARAM_UNUSED,
  ...
  FD_LOG_WARNING(( "*** Repair profiler topo ***" ));
  memset( &config->topo, 0, sizeof(config->topo) );
  repair_topo( config );

  FD_LOG_WARNING(( "*** Repair profiler init ***" ));
  ...
  FD_LOG_WARNING(( "*** Repair profiler run ***" ));
  fd_topo_run_single_process( topo, 2, config->uid, config->gid, fdctl_tile_run, NULL );
  for(;;) pause();
}

static void
repair_cmd_perm( args_t *         args   FD_PARAM_UNUSED,
                 fd_cap_chk_t *   chk    FD_PARAM_UNUSED,
                 config_t const * config FD_PARAM_UNUSED ) {}

static void
repair_cmd_args( int *    pargc FD_PARAM_UNUSED,
